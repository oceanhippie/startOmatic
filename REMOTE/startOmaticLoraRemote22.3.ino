/* startOmatic Remote by Tom Griffiths
https://www.ocenhippie.net/content/guides/startOmatic/
https://github.com/oceanhippie/startOmatic
*/
byte localAddress = 0xB0;     // address of this device ( E1-E4 = club Elliots B1 - start boat B0 = SHAW THING)
char BoatLetters[2] = "ST";  //Boatletters String ST = SHAW THING 
int mode = 1; //1 5R, 2 5U, 3R, 4 3U, 4 9U, 5 ST, 6 ET
...
const unsigned long period = 1000; // 1000 is one second 
...
const byte RecChars = 48;        //MAX Recieve String Lenght Make Sure you dont go longer than this weird stuff Happens
...
// Variables to store individual numbers (think this is legacy)
int  hourUnit=0;
int  minTens=0;
int  minUnits=0;
int  secTens=0;
...
//buttons descriptions are for Lora node v1

int Button1 = A1;       //far right header A1 GND 3.3v     MODE      
int Button2 = A0;       //second header A0 GND 3.3v        START
int Button3 = 4;       //third header D4 GND 3.3v        RESET 
int Button4 = 3;       //fourth header D3 GND 3.3v    HOOT
int Button5 = 5;       //bottom outer solder (square)       SPARE
int Button6 = 6;       //seconed to bottom outer solder (round)    SPARE
...
//horn is ALWAYS availabe
 ...
 //local v remote mode if the Dispaly and us are in diffenrert modes send a message to fix it 
 if(mode != dmode && millis() - lastSendTime > interval) {
    ...
    //OK make sure the remote is doing what we are
  if(stat != dstat) {
...
if (stat!=0) { //running
    if(digitalRead(Button2)  == LOW) { //we're running just send a finish
      if(mode==6 && FinishTimeToSend == false) {  mattFinishTime(); } else { Serial.println("Finish Sent");sendMessage(FinishMessage);}
       lcd.setCursor(2,1);
      lcd.print("Sent Finish     ");
 }  
...
//legace Elliot Start Timer code
...
if(strcmp(messageHedder, "Mode")==0) // Reply to change mode Message
  {
      String messageData2Temp(messageData2);
...
if(strcmp(messageHedder, "Time")==0) // Time update from Dispaly update our clock
  {
      String messageData2Temp(messageData2);
 ...
 if(strcmp(messageHedder, "Reset")==0) // Recive Reset from Display
  {
...
//legacy Elliot  Race Timer Timer Code
void mattFinishTime() {
          {
